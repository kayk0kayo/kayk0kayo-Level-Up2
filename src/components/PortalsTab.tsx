...
import { PixelSpriteCanvas } from "./PixelSpriteCanvas";
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col items-center gap-2 z-10 select-none bottom-3 relative">
              ...
              >
                {/* Renderização em pixel-art pura por Canvas se for Rank E (Zumbi) */}
                {activeCombat.monsterRank === 'E' ? (
                  <PixelSpriteCanvas 
                    spritesheetUrl="/zombie_sprite.png" // Utilize "zombie_sprite.png" na pasta "public" contendo as 6 sprites horizontais! (ex: tamanho 32x32px de frame, o arquivo seria 192x32px)
                    frameWidth={32}
                    frameHeight={32}
                    maxFramesX={6}
                    stateFrameY={0} // Sempre na linha 0 pois vamos assumir 1 única animação caso n tenha outras (Pode ter mais linhas caso queira)
                    animationSpeed={150}
                    scale={3} // Redimensionamento direto no canvas desativando anti-aliasing
